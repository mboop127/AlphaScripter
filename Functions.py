...
def mutate_ai(ai):
    ...
    for i in range(len(ai[0])):
        local[0][i] = mutate_simple(local[0][i])

    for i in range(len(ai[1])):
        local[1][i] = mutate_rule(local[1][i])
...
def generate_simple():
    ...
    type = random.choice(['train','build','research'])
...
def write_simple(simple):
    ...
    if type == 'build':
        ...
        string += "\n\t(building-type-count " + str(params['Buildable'] + " < " + str(threshold) + ")")
