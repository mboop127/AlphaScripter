...
def generate_ai():
    ...
    if use_preset:
        ...
        temp_facts[0] = ['true',0,temp_facts[0][2],'and']

        params = generate_parameters()
        params['TechId'] = '101'

        temp_actions[0] = ['research',params]

        preset = [1,1,["",""],temp_facts,temp_actions]
...
def write_ai(ai, ai_name):
    ...
    default = "(defrule\n(true)\n=>\n(set-strategic-number sn-cap-civilian-explorers 0)\n(set-strategic-number sn-initial-exploration-required 8)\n(disable-self))\n\n"
    #default = ""
...
def write_simple(simple):
    ...
    if type == 'build':
        ...
        if params['Buildable'] != 'farm':
            ...
            string += "\n\t(building-type-count " + params['Buildable'] + " < " + str(threshold) + ")"
...
def generate_distribution():
    ...
    villager_distribution = [[.25,.25,.25,0,.2],[.25,.25,.25,0,.2],[.25,.25,.25,0,.2],[.25,.25,.25,0,.2]]
...
def write_attack_rule(rule):
    ...
    if age_required[0] == "" and enemy_age_required[0] == "" and population1[0] == "" and population2[0] == "" and gametime[0] == "":
        string += "\n\t(true)"
    else:
        ...
        if enemy_age_required[0] != "":
            string += "(" + enemy_age_required[0] + ")"
            string += "\n\t"
