...
def generate_ai():
    ...
    return [villager_distribution,simple_list,ai]
...
def mutate_distribution(villager_distribution):

    for i in range(5):
        if random.random() < mutation_chance:
            villager_distribution[i] *= random.uniform(.95,1.05)
            villager_distribution[i] = min(1,max(0,villager_distribution[i]))
...
def write_distribution(villager_distribution):
    ...
    total = villager_distribution[0] + villager_distribution[1] + villager_distribution[2] + villager_distribution[3]

    for i in range(4):
        villager_distribution[i] = int(villager_distribution[i]/total * 100)

    villager_distribution[4] = int(villager_distribution[4] * 100)

    string = "\n(defrule\n\t(true)\n=>"
    string += "\n\t(set-strategic-number sn-food-gatherer-percentage " + str(villager_distribution[0]) + ")"
    string += "\n\t(set-strategic-number sn-wood-gatherer-percentage " + str(villager_distribution[1]) + ")"
